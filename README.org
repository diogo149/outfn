...
- I wish my code failed faster when I have a typo or refactoring error.
...
** Status
This is all very alpha, and we're still testing how much we like using this. Feedback would be very much appreciated though. (A better name would be cool too, since the ~outfn~ macro generates a macro to get the validation before runtime.)
** Features
*** vanilla/base outfn
...
- solves the problem of dependency passing (normal solutions involve explicitly declaring dependencies of all your dependencies or passing an opaque options map or using dynamic vars; parameters are coupled either way)
  ...
  - let's have an example:
    #+BEGIN_SRC clojure
      ;; let's start with two functions... they're all nice and understandable
      (defn gimme-data
        "Returns raw data"
        [db some-options]
      ...
      ;; in order to know how to call this function, I need to know how to call
      ...
      (defn api-call-coupled2
        ...
        (let [db :just-kidding
              some-options {:something :something}]
          ;; do I need to have all my db calls in my top-level calls just to have
          ;; decoupled code?
...
- easier debugging
  - try-catch can be automatically generated, and all the relevant information can be thrown on an exception
- optimizations
  - batching (eg. group together DB calls)
  - parallelization (evaluating the DAG on function calls)
...
- when multiple matches are possible, but not an exact match, the first match is chosen:
  #+BEGIN_SRC clojure
    (defoutfn outfn1
      "Docstring"
      ([foo] {:foo foo})
