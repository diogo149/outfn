...
** TODOs
- test implicit try+-catch
- test implicit try+-catch intermediates with lazy results
- test implicits + validation
- consider outputs in the map for the function being called implicitly, so that params can be cleaner and re-used
